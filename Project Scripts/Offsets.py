...
Functions = [
	...
	["CastSpell", "E8 C3 50 F3 FF 5E 85 DB 74 1C 8B CF E8 87 64 F0 FF 84 C0 74 11 8B 0D 20 4C 57 01 FF 73 08 8B ", 2], # ALE-229C053F / ERROR: Client Tried to cast a spell from an invalid slot: %d
...
#Chat	
	...
	["OpenChat", "83 EC 30 A1 C0 53 56 01 33 C4 89 44 24 2C 53 57", 1], # Can be found with "game_console_chatcommand_allchat_1"
	
	["DisplayChat", "81 EC 3C 01 00 00 A1 C0 53 56 01 33 C4 89 84 24 38 01 00 00 55", 1], #Can be found with "MaxChatBufferSize"
	
	["SendChat", "56 6A FF FF 74", 1],
	#\x56\x6A\xFF\xFF\x74 xxxxx
	
	["SetChatScale", "83 EC 24 A1 C0 53 56 01 33 C4 89 44 24 20 F3 0F", 1], #Can be found with "SetChatScaleX"
	
	["IssueOrder", "E8 68 47 C1 FF ? ? ? ? ? ", 2], #Can be found in EventMoveMouseClick, the caller of the caller of CreatePath
	...
	["OnSpellCast", "81 EC FC 04 00 00 A1 C0 53 56 01 33 C4 89 84 24 F8 04 00 00 53 8B 9C 24 04 05 00 00 55 56 8B ", 1],
	
	["OnAutoAttack", "83 EC 08 53 8B D9 53 ", 1],
	...
	["InitEvent", "83 EC 4C A1 C0 53 56 01 33 C4 89 44 24 48 53 55", 1], #Can be found with "CloseOnEndGameDelaySeconds" and following the jump if not zero  first call and then the second function within that call is InitEvent
	
	["OnLoadHero", "81 EC 84 00 00 00 A1 C0 53 56 01 33 C4 89 84 24 80 00 00 00 57 8B", 1], #Can be found with {:5s}| {#channel!} Spawn Champion Request {ChampionName} [{#Summo
	
	["OnLoadChampion", "83 EC 08 E8 38 27 64 00 ? ? ? ? ? ", 1], #Found within OnLoadHero
	["OnLoadChampion", "83 EC 08 E8 98 1E 63 00 ? ? ? ? ? ", 1],
	...
	["GameUpdate", "81 EC 4C 01 00 00 A1 C0 53 56 01 33 C4 89 84 24 48 01 00 00 55 56 8B F1", 1], #Can be found with "Waiting for all players to connect\n"
...
Offsets = [
	["LocalPlayer","39 3D 38 FF E4 02 ? ? ? ? ? ? ", 1, 0],
	...
	["GameState", "A3 FC 9A E1 02 ? ? ? ? ? ", 1, 1], #Can be found in GameUpdate above the loc with "Game Ended.  Closing after delay..." and 3 renderer references
